...
const AddForm = ({ 
  ...
  const handleSubmit = () => {
    if (formData.title && formData.ceo && formData.offices) {
      onSubmit(formData);
      resetForm();
      onClose();
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div 
        ref={modalRef}
        className="bg-white rounded-lg shadow-xl w-[70%] mx-4 max-h-[90vh] overflow-y-auto"
      >
        {/* Header */}
        ...
        <div className="px-6 py-2">
          <div className="space-y-4 rounded-lg ">
            ...
            {/* Workspace Seats Select */}
            <select
              name="workspaceSeats"
              value={formData.workspaceSeats}
              onChange={handleInputChange}
              className="w-full px-3 py-2 placeholder-gray-500 bg-transparent border rounded-lg border-lightGray focus:outline-none appearance-none bg-[url('data:image/svg+xml,%3csvg xmlns=%27http://www.w3.org/2000/svg%27 fill=%27none%27 viewBox=%270 0 20 20%27%3e%3cpath stroke=%27%236b7280%27 stroke-linecap=%27round%27 stroke-linejoin=%27round%27 stroke-width=%271.5%27 d=%27M6 8l4 4 4-4%27/%3e%3c/svg%3e')] bg-no-repeat bg-right-1"
              required
            >
              <option value="">Workspace Seats</option>
              {["1-10", "11-25", "26-50", "51-100", "100+"].map((seat, index) => (
                <option key={index} value={seat} className="text-gray-900">
                  {seat}
                </option>
              ))}
            </select>

            {/* Date of Tendency Input */}
            <input
              type="date"
              name="dateOfTendency"
              value={formData.dateOfTendency}
              onChange={handleInputChange}
              placeholder="Date of Tendency"
              className="w-full px-3 py-2 placeholder-gray-500 bg-transparent border rounded-lg border-lightGray focus:outline-none"
              required
            />

            {/* Period of Tendency Input */}
            <input
              type="text"
              name="periodOfTendency"
              value={formData.periodOfTendency}
              onChange={handleInputChange}
              placeholder="Period of Tendency"
              className="w-full px-3 py-2 placeholder-gray-500 bg-transparent border rounded-lg border-lightGray focus:outline-none"
              required
            />
          ...
          {/* Action Buttons */}
          <div className="flex flex-col items-center justify-center w-full mt-6 md:flex-row md:justify-evenly gap-y-2 md:gap-x-4 lg:w-[45%] mx-auto">
            <button
              type="button"
              onClick={handleSubmit}
              className="w-full px-2 py-2 font-medium custom-AddButton text-white transition-colors rounded-lg md:w-64 custom-ActionBtn "
            >
              Add
            </button>
            <button
              ...
              className="px-2 py-2 w-full  md:w-64 font-medium text-gray-700 transition-colors rounded-lg bg-gray-200
               hover:bg-gray-300"
            >
              Cancel
            </button>
